...
QUICK_TOUR: list[Step] = [
    Step(wait_for="Resources"),  # Home screen renders
    Step(keys=("down",)),  # highlight Identities
    Step(keys=("enter",), wait_for="Unix username"),  # IdentityListScreen (column header)
    Step(keys=("enter",), wait_for="Unix username"),  # IdentityViewScreen for row 0 (border title)
    ...
    Step(keys=("enter",), wait_for="Time"),  # AuditLogListScreen (column header)
    Step(keys=("escape",), wait_for="Resources"),  # back to Home
...
THOROUGH_TOUR: list[Step] = [
    ...
    Step(wait_for="Reconnecting", timeout=10.0),
    Step(wait_for="Resources", timeout=15.0),
    # Tenants (index 0)
    Step(keys=("enter",), wait_for="Display Name", timeout=8.0),
    Step(keys=("a",), wait_for="Add a tenant"),
    Step(keys=("acme", "tab", "Acme Corp", "enter"), wait_for="acme", timeout=8.0),
    Step(keys=("escape",), wait_for="Resources"),
    # Identities (index 1): create, then invite (seeded throwaway secret)
    Step(keys=("down", "enter"), wait_for="Unix username", timeout=8.0),
    Step(keys=("a",), wait_for="Add an identity"),
    Step(keys=("dana", "enter"), wait_for="Unix username", timeout=8.0),  # auto-opens IdentityViewScreen
    ...
    Step(keys=("enter",), wait_for="Invitation secret", timeout=8.0),  # method "manual" (default)
    Step(keys=("escape",), wait_for="dana"),  # dismiss secret, back to list
    Step(keys=("escape",), wait_for="Resources"),
    # Bastions (index 2)
    Step(keys=("down", "enter"), wait_for="SSH Proxy Jump", timeout=8.0),
    ...
    Step(keys=("escape",), wait_for="Resources"),
    # Boundaries (index 3)
    Step(keys=("down", "enter"), wait_for="Ceiling", timeout=8.0),
    Step(keys=("a",), wait_for="Add a boundary"),
    Step(keys=("demo-zone", "enter"), wait_for="demo-zone", timeout=8.0),  # auto-opens BoundaryViewScreen
    Step(keys=("escape",), wait_for="Ceiling", timeout=8.0),  # back to BoundaryListScreen
    Step(keys=("escape",), wait_for="Resources"),
    # Tags (index 4)
    Step(keys=("down", "enter"), wait_for="Value", timeout=8.0),
    Step(keys=("a",), wait_for="Add a tag"),
    Step(keys=("env", "tab", "demo", "enter"), wait_for="env", timeout=8.0),
    Step(keys=("escape",), wait_for="Resources"),
    # Roles (index 5): create, add two grant types to the grant editor
    Step(keys=("down", "enter"), wait_for="Members", timeout=8.0),
    Step(keys=("a",), wait_for="Add a role"),
    Step(keys=("demo-role", "enter"), wait_for="Grants", timeout=8.0),  # auto-opens RoleViewScreen
    Step(keys=("tab", "tab", "tab")),  # focus the (empty) grants table
    Step(keys=("a",), wait_for="Add grant"),
    Step(keys=("enter",), wait_for="Permissions", timeout=8.0),  # grant type "identity" (default cursor)
    Step(keys=("ctrl+s",), wait_for="identity", timeout=8.0),  # confirm defaults, back to RoleViewScreen
    Step(keys=("a",), wait_for="Add grant"),
    Step(keys=("down", "enter"), wait_for="Permissions", timeout=8.0),  # grant type "tag" (index 1)
    ...
    Step(keys=("escape",), wait_for="Resources"),
    # Auths (index 6)
    Step(keys=("down", "enter"), wait_for="Enabled", timeout=8.0),  # AuthListScreen ("Enabled" column)
    Step(keys=("a",), wait_for="Auth type"),
    Step(keys=("enter",), wait_for="New http_sig auth"),  # default cursor on http_sig
    Step(keys=("demo-auth", "tab", "cli", "enter"), wait_for="Enabled", timeout=8.0),  # auto-opens AuthViewScreen
    Step(keys=("escape",), wait_for="Client Type", timeout=8.0),  # back to AuthListScreen
    Step(keys=("escape",), wait_for="Resources"),
    # Audit Log (index 7)
    Step(keys=("down", "enter"), wait_for="Time", timeout=8.0),
    Step(keys=("escape",), wait_for="Resources"),
    Step(keys=("escape",)),  # app.quit
]
