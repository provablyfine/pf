  $ idbctl ssh key generate --format openssh > ed25519.key
  $ chmod og= ed25519.key
  $ ssh-keygen -l -f ed25519.key
  256 SHA256:[^(]+\(ED25519\) (re)

  $ idbctl ssh key generate -t rsa-3072 --format openssh > rsa-3072.key
  ...
  $ idbctl ssh key generate -t ecdsa-384 --format openssh > ecdsa-384.key
  $ chmod og= ecdsa-384.key
  $ ssh-keygen -l -f ecdsa-384.key
  384 SHA256:[^(]+\(ECDSA\) (re)

  $ idbctl ssh key generate -t ecdsa-521 --format openssh > ecdsa-521.key
