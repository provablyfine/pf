...
def _already_accepted(
    invitation: model.identity_invitation_key.IdentityInvitationKey, account_key: jwk.Public
...
def accept_invitation_endpoint(
    ...
    if invitation.is_accepted:
        return _already_accepted(invitation, account_key)

    # all verification passed. Bind the public account key with the identity
    # that was configured in the invitation.
    if not model.identity_invitation_key.accept(
        id=invitation.id,
        public_key_id=account_key.thumbprint(),
    ):
        # Someone else got there between our check and our write.
        current = model.identity_invitation_key.read(invitation.id)
        if current is not None and current.is_accepted:
            return _already_accepted(current, account_key)
        return responses.problem_response(status_code=403, title="Invitation is no longer valid")
    now = int(time.time())
