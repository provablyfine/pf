...
def _str_to_tags(value: str) -> list[dict]:
    return [{"name": k, "value": v} for k, v in (s.split("=", 1) for s in value.split() if "=" in s)]
...
class AuthViewScreen(textual.screen.Screen[None]):
    ...
    def __init__(self, auth: async_client.AsyncClient, a: dict) -> None:
        ...
        self._saved_tags_str: str = _tags_to_str(a.get("tags", []))
    ...
    def compose(self) -> textual.app.ComposeResult:
        ...
        with textual.containers.Vertical():
            ...
            if self._a["type"] == "oidc":
                params = self._a.get("params", {})
                with textual.containers.HorizontalGroup(classes="field") as c:
                    c.border_title = "Issuer"
                    yield textual.widgets.Input(params.get("issuer", ""), id="issuer", compact=True)
                with textual.containers.HorizontalGroup(classes="field") as c:
                    c.border_title = "Client ID"
                    yield textual.widgets.Input(params.get("client_id", ""), id="client_id", compact=True)
                with textual.containers.HorizontalGroup(classes="field") as c:
                    c.border_title = "Client secret"
                    yield textual.widgets.Input(
                        "", placeholder="unchanged", id="client_secret", compact=True, password=True
                    )
            elif self._a["type"] == "oauth2-github":
                params = self._a.get("params", {})
                with textual.containers.HorizontalGroup(classes="field") as c:
                    c.border_title = "Authorization endpoint"
                    yield textual.widgets.Label(params.get("authorization_endpoint", ""))
                with textual.containers.HorizontalGroup(classes="field") as c:
                    c.border_title = "Callback URL"
                    yield textual.widgets.Label(params.get("callback_url", ""))
    ...
    async def action_save(self) -> None:
        ...
        if self._a["type"] == "oidc":
            ...
            saved_params = self._a.get("params", {})
            params_changed = (
                issuer != saved_params.get("issuer", "")
                or client_id != saved_params.get("client_id", "")
                or client_secret
            )
            if params_changed:
        ...
        response = await self._auth.patch(f"{self._auth.directory.auth}/{self._a['id']}", json=patch)
