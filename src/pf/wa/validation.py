...
class Middleware(middleware.Middleware):
    def __call__(
        self, request: request.Request, iterator: collections.abc.Iterator[middleware.Middleware]
    ) -> response.Response:
        ...
        try:
            ...
            return response.ProblemResponse(
                status_code=400, title="Request invalid.", detail=f"{error['msg']}: {'.'.join(map(str, error['loc']))}"
            )
