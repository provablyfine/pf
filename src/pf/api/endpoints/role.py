...
def list_endpoint(name: str | None = None, id: int | None = None) -> fastapi.responses.Response:
    ...
    return fastapi.responses.JSONResponse(
        status_code=200,
        content=schemas.RoleListResponse(roles=roles).model_dump(),
    )
...
def create_endpoint(data: schemas.RoleCreateRequest) -> fastapi.responses.Response:
    ...
    return fastapi.responses.JSONResponse(
        status_code=201,
        content=converters.role_to_schema(converter, role).model_dump(),
    )


@router.delete("/{role_id:int}")
def delete_endpoint(role_id: int) -> fastapi.responses.Response:
    ...
    if not grants.role(role.id).can_delete():
        return responses.problem_response(status_code=403, title="Not allowed to delete role")

    member = ctx.db.role_member.read_one(role_id=role.id)
    if member is not None:
        return responses.problem_response(status_code=400, title="Role is still in use")

    ctx.db.role.delete(id=role.id)
    return fastapi.responses.Response(status_code=204)


@router.patch("/{role_id:int}")
def update_endpoint(role_id: int, data: schemas.RoleUpdateRequest) -> fastapi.responses.Response:
    ...
    for field_name in data.model_fields_set:
        if not grants.role(role.id).can_update(field_name):
            return responses.problem_response(
                status_code=403, title="Not allowed to update role field", detail=field_name
            )
    ...
    if "member_list" in data.model_fields_set:
        ...
        if ctx.identity_id in deleted_member_id_list:
            return responses.problem_response(status_code=403, title="Not allowed to remove self from role")
