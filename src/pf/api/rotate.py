...
def main():
    ...
    with engine.begin() as connection:
        ...
        with ctx.set_db(dao), ctx.set_kek(kek):
            rotate(db.SigningKeyType.HOST, jwk.KeyType.from_string(conf.host_key_type), conf.host_key_rotation_period, conf.host_key_staging_period)
            rotate(db.SigningKeyType.USER, jwk.KeyType.from_string(conf.user_key_type), conf.user_key_rotation_period, conf.user_key_staging_period)
